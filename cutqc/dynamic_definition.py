...
from helper_functions.metrics import MSE, fidelity
...
def full_verify(full_circuit, complete_path_map, subcircuits, dd_bins):
    ...
    # print('prb', real_probability)
    # print('nb', full_circuit.num_qubits)
    approximation_error = (
        MSE(target=ground_truth, obs=real_probability)
        * 2**full_circuit.num_qubits
        / np.linalg.norm(ground_truth) ** 2
    )
    fidelity_score = fidelity(target=ground_truth, obs=real_probability)
    print('Approximation Error:', approximation_error)
    print('Fidelity:', fidelity_score)
    return reconstructed_prob, ground_truth, fidelity_score
