...
def cuts_parser(cuts, circ):
    ...
    for position in cuts:
        ...
        for gate_idx, gate in enumerate(
            ...
            if len(gate.qargs) > 1:
                gate_alias = gate.name.upper() + "(" + ", ".join(str(q.index) for q in gate.qargs) + ")"
                if gate_alias in gate_occurrence_map:
                    gate_occurrence_map[gate_alias] += 1
                else: gate_occurrence_map[gate_alias] = 1
                tmp += 1
                if tmp == multi_Q_gate_idx:
                    gate_to_cut = (gate.name.upper(), gate.qargs, gate_occurrence_map[gate_alias])
                    all_Q_gate_idx = gate_idx
        positions.append((wire, all_Q_gate_idx))
        positions_mlft.append((wire, gate_to_cut))
    positions = sorted(positions, reverse=True, key=lambda cut: cut[1])
    positions_mlft = sorted(positions_mlft, reverse=True, key=lambda cut: cut[1][2])
    return positions, positions_mlft
...
def find_cuts(
    ...
    for num_subcircuit in num_subcircuits:
        ...
        if feasible:
            positions, positions_mlft = cuts_parser(mip_model.cut_edges, circuit)
