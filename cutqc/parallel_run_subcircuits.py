...
if __name__ == "__main__":
    ...
    for instance_init_meas in rank_jobs:
        ...
        for meas in mutated_meas:
            ...
            print(subcircuit_instance) # DELETE
            print(meas, measured_prob) # DELETE
            # from helper_functions.plot_data import plot_data # DELETE
            # plot_data(subcircuit_inst_prob, f'./out/{str(subcircuit_idx)+",".join(meas)+str(instance_init_meas_id)}.png', False) # DELETE
            pickle.dump(
                measured_prob,
                open(
